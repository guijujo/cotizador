...
function Presupuestos() {
  ...
  const presupuestosActuales = presupuestos.slice(
  ...
  const eliminarPresupuesto = (indice) => {
    setPresupuestos((presupuestosActuales) =>
      presupuestosActuales.filter((_, i) => i !== indice)
    );
  };
  ...
  return (
    <>
      ...
      {presupuestos.length > 0 && (
        <button id="eliminarHistorial" onClick={borrarHistorial}>Borrar historial</button>
      )}
      <ul>
        {presupuestosActuales.map((elemento, indice) => (
          <Presupuesto
            key={indice}
            {...elemento}
            onDelete={() => eliminarPresupuesto(indice)}
          />
        ))}
      </ul>
      {presupuestos.length > 0 && (
        <div id="paginacion">
          ...
          {Array.from({ length: paginas }, (_, i) => i + 1).map(
            (numeroPagina) => (
              <button
                key={numeroPagina}
